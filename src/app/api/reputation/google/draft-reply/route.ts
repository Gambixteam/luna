...
export async function POST(request: NextRequest) {
  try {
    ...
    const review = await context.supabase.from('reputation_reviews').select('*,business_profiles!inner(*)').eq('id', body.reviewId).eq('organization_id', context.organizationId).maybeSingle();
    let reviewData = review.data as Record<string, any> | null;
    if (!reviewData) {
      const direct = await context.supabase.from('reputation_reviews').select('*').eq('id', body.reviewId).eq('organization_id', context.organizationId).single();
      if (direct.error || !direct.data) throw new Response('Review not found.', { status: 404 });
      const profile = await context.supabase.from('business_profiles').select('*').eq('organization_id', context.organizationId).single();
      reviewData = { ...direct.data, businessProfile: profile.data };
    }
    ...
    const generated = await generateLunaJson({
      ...
      prompt: JSON.stringify({ reviewer: reviewData.reviewer_name, rating: reviewData.star_rating, review: reviewData.comment, business: reviewData.businessProfile ?? null, requiredShape: { reply: 'string' } }),
    ...
    if (!existing.data) {
      await context.supabase.from('approvals').insert({ organization_id: context.organizationId, site_id: reviewData.site_id, approval_type: 'content', resource_type: 'reputation_review', resource_id: reviewData.id, title: `Review response: ${reviewData.reviewer_name ?? 'Google reviewer'}`, summary: reviewData.comment ?? 'Review without written comment', proposed_state: { reply }, requested_by: context.user.id });
    }
