...
export async function POST(request: NextRequest) {
  try {
    ...
    const completedRuns = await context.supabase.from('audit_runs').select('id').eq('site_id',site.id).eq('status','completed').order('created_at',{ascending:false}).limit(5);
    const completedRunIds = completedRuns.data?.map((item)=>item.id) ?? [];
    const [profileResult,integrationsResult,citationsResult,reviewsResult,keywordsResult,pagesResult,competitorsResult] = await Promise.all([
      ...
      completedRunIds.length ? context.supabase.from('audit_pages').select('*').in('audit_run_id',completedRunIds) : Promise.resolve({data:[],error:null}),
    ...
    const gbpLocations = (metadata.locations ?? []) as Array<Record<string,any>>;
    const selected = gbpLocations.find((item)=>item.name===metadata.selectedLocation) ?? (gbpLocations.length===1?gbpLocations[0]:null);
    ...
    else if(!selected) add(findings,'high','GBP location selection is incomplete','Choose the correct business location before profile recommendations or publishing.',profileUrl,{locationsFound:gbpLocations.length});
    if(selected){
    ...
    const serviceAreas=(profile.service_areas ?? []) as string[];
    const pageText=pages.map((item)=>`${item.url} ${item.title ?? ''} ${item.h1 ?? ''}`.toLowerCase());
    const uncovered=serviceAreas.filter((location)=>!pageText.some((text)=>text.includes(location.toLowerCase())));
    if(serviceAreas.length && uncovered.length) add(findings,'medium','Target locations lack clear landing-page coverage','Create useful, differentiated location content only where the business can genuinely serve customers.',profileUrl,{uncoveredLocations:uncovered});
    ...
    const summary={score:Math.max(0,100-findings.reduce((total,item)=>total+weight[item.severity],0)),gbpConnected:gbp?.status==='connected',gbpLocations:gbpLocations.length,citations:citations.length,citationIssues:missingCitations.length,reviews:reviews.length,averageRating:average,reviewReplyRate:reviews.length?replied/reviews.length:null,localKeywords:localKeywords.length,uncoveredLocations:uncovered,competitors:competitors.length};
